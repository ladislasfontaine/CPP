...
/*   Updated: 2020/07/17 16:13:07 by lafontai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				Bureaucrat::executeForm(const Form &form) const {
	if (this->getGrade() <= form.getGradeToExecute() && form.getSigned())
		std::cout << _name << " executs " << form.getName() << std::endl;
	else if (this->getGrade() > form.getGradeToExecute())
		std::cout << _name << " cant execute " << form.getName() << " because he needs at least grade " << form.getGradeToExecute() << "." << std::endl;
}
